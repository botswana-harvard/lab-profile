...
class LabProfile(object):
    ...
    # controller uses this to register the aliquot, receive,
    # ... models so they can be referenced by the group name
    ...
    def specimen_identifier(self, requisition):
        """Returns a globally unique specimen identifier based on the requisition
        identifier."""
        ...
        if not specimen_identifier:
            raise SpecimenError(
                'Cannot set specimen identifier for the specimen in Requisition {}. '
    ...
    def aliquot(self, source_aliquot_or_pk, aliquot_type, count):
        """Creates aliquots from the source aliquot and increments
        the aliquot count from the existing primary."""
        try:
    ...
    def aliquot_identifier(self, instance, aliquot_type, aliquot_count):
        """Returns an aliquot identifier create from a source aliquot instance or,
        if PRIMARY, from the receive instance identifier.

        Args:
            instance: either an aliquot or receive instance."""
        aliquot_stub = '{0}{1}'.format(aliquot_type.numeric_code.zfill(2), str(aliquot_count).zfill(2))
        try:
            return '{0}{1}'.format(instance.receive.receive_identifier, aliquot_stub)
        except AttributeError:
            return '{0}{1}'.format(instance.receive_identifier, aliquot_stub)
